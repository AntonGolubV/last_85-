...
function dataSmartphone(brand) {
  ...
  switch (brand) {
    case "apple":
      smartFilteredObj = filterObjects(dataApple.products, [
        "id",
        "key",
        "extended_name",
        "images",
      ...
      break;
    case "samsung":
      smartFilteredObj = filterObjects(dataSamsung.products, [
        "id",
        "key",
        "extended_name",
        "images",
      ...
      break;
    case "honor":
      smartFilteredObj = filterObjects(dataHonor.products, [
        "id",
        "key",
        "extended_name",
        "images",
      ...
      break;
    case "realme":
      smartFilteredObj = filterObjects(dataRealme.products, [
        "id",
        "key",
        "extended_name",
        "images",
      ...
      break;
    case "poco":
      smartFilteredObj = filterObjects(dataPoco.products, [
        "id",
        "key",
        "extended_name",
        "images",
      ...
      break;
    case "vivo":
      smartFilteredObj = filterObjects(dataVivo.products, [
        "id",
        "key",
        "extended_name",
        "images",
      ...
      break;
    case "xiaomi":
      smartFilteredObj = filterObjects(dataXiaomi.products, [
        "id",
        "key",
        "extended_name",
        "images",
      ...
      break;
    case "huawei":
      smartFilteredObj = filterObjects(dataHuawei.products, [
        "id",
        "key",
        "extended_name",
        "images",
...
function filterByParam(obj) {
  ...
  for (let i = 0; i < arrBrand.length; i++) {
    for (let k = 0; k < arrBrand[i].length; k++) {
      if (
        arrBrand[i][k].prices != null &&
        parseInt(arrBrand[i][k].prices.price_max.amount) >=
          parseInt(obj.price[0]) &&
        parseInt(arrBrand[i][k].prices.price_max.amount) <=
          parseInt(obj.price[1])
      ) {
...
function check(newArr, obj, strArr, obj_param) {
  if (
    parseInt(obj_param[0]) <= parseInt(strArr) &&
    parseInt(obj_param[1]) >= parseInt(strArr)
  ) {
    newArr.push(obj);
  }
  return newArr;
...
function getDeviceBySubname(subname) {
  ...
  for (let i = 0; i < allData.length; i++) {
    for (let k = 0; k < allData[i].products.length; k++) {
      if (
        allData[i].products[k].full_name
          .toLowerCase()
...
function deleteSmart(id, arr){
  for(let i = 0; i < arr.length; i++){
    if(parseInt(id) == arr[i].id){
      arr.splice(i, 1);
      return arr;
    }
  }
...
function searchSmartByID(id, arr){
  for(let i = 0; i < arr.length; i++){
    if(parseInt(id) == arr[i].id){
      return arr[i];
    }}
